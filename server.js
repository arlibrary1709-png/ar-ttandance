...
const VAPID_EMAIL   = process.env.VAPID_EMAIL || 'mailto:admin@arlibrary.com';
...
// ─── PUSH SUBSCRIPTION ───────────────────────────────────────
// GET public VAPID key (frontend needs this)
...
app.post('/api/push/subscribe', async (req, res) => {
  try {
    ...
    await PushSubscription.findOneAndUpdate(
      ...
      { subscription, member_id: member_id || null },
...
// Send push to all subscribers (called when admin publishes notice)
app.post('/api/push/send', async (req, res) => {
  try {
    ...
    const payload = JSON.stringify({ title, body, tag: tag || 'ar-notice' });
    let sent = 0, failed = 0;
